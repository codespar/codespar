...
This repo (`codespar/codespar`, public, MIT) is the **open-source self-hostable runtime**. A managed tier (private, commercial) registers commerce-governance capabilities against the plugin hooks exposed here.

**Thesis:** codespar is MIT-licensed LATAM commerce-agent infrastructure — a self-hostable runtime, SDK, and MCP catalog, with a managed tier above the free OSS layer.

**Five-point MIT commitment** (non-negotiable):
...
| `codespar/mcp-dev-latam` | Public, MIT | LATAM MCP server catalog (payments, fiscal, logistics, messaging) |
| Managed tier | Private, commercial | Implements the same session contract; registers commerce-governance against the plugin hooks here |

**Dependency arrow:** strictly managed → MIT. The public core never imports managed-tier packages. Plugin hooks (`PolicyHook`, `ObservabilityHook`, `SecretsHook`, `IntegrationHook`, `MetaToolHook`) are the integration surface; the managed tier registers implementations at runtime.

## What this runtime ships

The runtime is **the OSS implementation of the session contract** defined in [`@codespar/api-types`](https://www.npmjs.com/package/@codespar/api-types). The same contract is implemented by the managed tier, so an agent written against `@codespar/sdk` runs unchanged against either endpoint — `baseUrl` is a configuration change, not a rewrite.
...
- **Projects API** (`packages/core/src/server/routes/projects.ts`) — CRUD for the 2-level tenancy model (Org → Project), mirroring the contract the managed tier serves.
...
- **Plugin hooks** (`packages/core/src/plugins/`) — `PolicyHook`, `ObservabilityHook`, `SecretsHook`, `IntegrationHook`, `MetaToolHook`. The managed tier registers implementations here.
...
The OSS runtime is functionally complete for self-hosting an agent that transacts on LATAM rails. The following surfaces are not yet in OSS and are on the roadmap:

- **MCP server catalog API** (`/v1/servers`). In OSS today, providers are registered manually via SDK config.
- **Connections vault** (`/v1/connections`, `/v1/auth-configs`). In OSS today, credentials live in environment variables.
- **Programmable wallet + policy engine**.
- **Commerce-specific observability**. Generic trace infrastructure (Pino logger, metrics) exists; commerce-flavored observability is on the roadmap.

When these surfaces land in OSS, the dependency arrow stays the same — managed → MIT.
...
User-facing docs are deployed at [docs.codespar.dev](https://docs.codespar.dev); this repo does not ship a docs site of its own.
...
Mirrors the managed-tier contract. Every write path stamps `project_id`.
...
The integration surface for the managed tier (and any third party). Hooks live in `packages/core/src/plugins/`.

| Hook | Purpose |
|------|---------|
| `PolicyHook` | Pre-execution policy evaluation — gate sessions/tool calls based on org rules. The managed tier registers programmable wallet limits, mandate caps, deny-list enforcement. |
| `ObservabilityHook` | Trace + metric emission. The managed tier registers commerce-specific span enrichment (provider, currency, settlement). |
| `SecretsHook` | Provider credential retrieval. The managed tier registers vault-backed lookup. |
| `IntegrationHook` | Outbound integration registration (e.g. Sentry, Linear). |
| `MetaToolHook` | Higher-level (meta) tool registration — register a named tool that the runtime dispatches by name through the standard execute path. |

**Rule:** `@codespar/core` never imports managed-tier packages. The managed tier loads at runtime via `pluginRegistry.register(...)`.
...
- Storage operations that hit typed error codes (`slug_conflict`, `cannot_delete_default`, etc.) translate to 4xx responses with the same `error` codes as the managed tier
...
- NEVER import from managed-tier packages in `@codespar/core`. The dependency arrow is strictly managed → MIT.
...
- NEVER store provider credentials in plaintext. In OSS, env vars; in the managed tier, a vault.
- NEVER auto-execute commerce actions that the safety contract gates (fund transfers above the configured cap, NF-e/CFDI/Factura issuance for contested carts, wallet-policy overrides, bulk outbound messaging above threshold, cross-tenant agent-to-agent commitments). These require human approval regardless of autonomy configuration.
...
MIT. The five-point commitment above is the binding contract.
