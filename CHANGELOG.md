...
- **Two replicas sharing a state directory disagree less often.** Both find no credential, both generate one, and both write; the rename is atomic, so one wins the file and the other is left holding a token that matches nothing and answers `401` to every caller. The generated credential is now read back after writing, and a replica that finds a different value adopts it. Read the limit precisely, because it is narrower than it sounds: a replica that loses the race **before** the read-back adopts the stored value and logs it, while one that loses **after** the read-back keeps a stale credential with no warning at all. This narrows the window, it does not make losing loud. **Set `ENGINE_API_TOKEN` when running more than one replica** — that removes the race rather than narrowing it. The effect of getting this wrong is availability (a stale replica rejects legitimate callers), not access: no invalid token ever becomes valid.
...
- **GitHub webhooks are now registered with a signing secret the runtime provisions itself.** Previously `createWebhook` sent no `secret`, so every hook the runtime created was delivered unsigned — and an operator who set `GITHUB_WEBHOOK_SECRET` to be responsible made it *worse*, because verification switched on while GitHub still sent nothing to verify, and deliveries began failing. The secret is now generated, stored, and sent to GitHub on creation; an operator-supplied value still wins over the generated one. Hooks registered by earlier releases are repaired at startup, one time per project, so existing installs become verifiable without re-registering anything by hand. The repair is best-effort and never blocks or fails a boot, and a project is only marked done when GitHub actually accepted the secret.

**Still not changed, and worth knowing before you expose this runtime.** The provider webhook routes (`/webhooks/github`, `/webhooks/vercel`, `/webhooks/sentry`, `/webhooks/deploy`) are not covered by the bearer credential, and unsigned deliveries are still **accepted** unless `WEBHOOK_STRICT_MODE=true`. An accepted payload is written to the audit log, broadcast over SSE, and dispatched to the event handlers the agents act on. Provisioning above is what makes flipping that default possible without breaking the runtime's own integration; the flip itself is a separate, deliberate decision tracked in [#138](https://github.com/codespar/codespar/issues/138). Until then, restrict who can reach those four routes at the network layer. Vercel and Sentry hooks are created in those providers' dashboards, so their secrets remain yours to configure.
