...
configured. All four webhook endpoints are supported:
...
| Deploy (generic) | `x-deploy-signature` | SHA256 | `DEPLOY_WEBHOOK_SECRET` |
...
DEPLOY_WEBHOOK_SECRET=your-deploy-secret
```
...
| `DEPLOY_WEBHOOK_SECRET` | No | Shared secret for generic deploy webhook verification |
