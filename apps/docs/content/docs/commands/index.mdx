...
description: Complete reference for all 21 CodeSpar agent commands, including syntax, risk levels, and usage examples.
---
...
| `plan` | `plan <feature>` | medium | Break down a feature into sequential sub-tasks via [Planning Agent](/docs/agents/planning-agent) |
| `lens` | `lens <question>` | low | Query data and get insights |
| `spec` | `spec <feature>` | medium | Phase 1: Generate structured requirements (EARS, user stories, acceptance criteria) |
| `spec design` | `spec design` | medium | Phase 2: Generate technical design from requirements |
| `spec tasks` | `spec tasks` | medium | Phase 3: Generate implementation plan from design |
| `spec status` | `spec status` | low | Show spec-driven development workflow |
