...
/**
 * Meta-tool registration seam — the fifth plugin hook.
 *
 * `MetaToolHook` and its `MetaToolExecutionContext`/`MetaToolDefinition`/
 * `MetaToolResult` companions are defined canonically in `@codespar/types`
 * (the zero-dependency, publishable contract package) and re-exported here so
 * hook authors import them from `@codespar/core` alongside the other four
 * plugin hooks. One definition, one source of truth; see `@codespar/types`
 * for the doc comments and field-level rationale.
 */
export type {
  MetaToolHook,
  MetaToolExecutionContext,
